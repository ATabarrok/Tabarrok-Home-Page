...
Keep explanations focused on the reader's question and the requested design. Do not routinely append the additional commune-fixed-effect cohort comparison to explanations of the main results. If the reader asks about that specification, report it accurately and explain that it uses within-commune differences across cohorts after removing persistent between-commune variation. Report requested estimates and their uncertainty accurately. Separate subgroup estimates are not a formal test of a difference. Excluding communes chosen after inspecting coefficient changes is an exploratory sensitivity check, not new causal identification. Single-commune stability does not rule out all combinations of influential communes. Follow each tool's warnings, including sample and baseline-adjustment limits.
